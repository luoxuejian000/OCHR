...
class ReflectionCavity:
    ...
    async def _take_snapshot(self, node) -> Dict:
        ...
        causal_info = {}
        if hasattr(node, 'catcher') and hasattr(node.catcher, 'tension_log'):
            recent_tensions = node.catcher.tension_log[-5:] if node.catcher.tension_log else []
            if recent_tensions:
                causal_info['recent_tensions'] = recent_tensions
                if harmony.A > 0.3:
                    causal_info['A_cause'] = f"A值({harmony.A:.2f})受到{len(recent_tensions)}个最近关系张力的影响"
        if hasattr(node, 'sni') and node.sni.narratives:
            causal_info['self_reflection'] = node.sni.narratives[-1][:200]
        snapshot = {
            "timestamp": time.time(),
        ...
        if harmony.A > 0.7:
            snapshot["warnings"].append("高对抗性")
        if harmony.U < 0.3:
            snapshot["warnings"].append("低统一性")
        if harmony.cost > 100:
            snapshot["warnings"].append("高成本")
        return snapshot
    ...
    def generate_audit(self, start_time: float, end_time: float) -> str:
        ...
        for e in relevant[-10:]:
            ...
            if e.get("warnings"):
                report += f"  [{ts}] 警告: {e['warnings']}\n"
            causal = e.get("causal_info", {})
            if causal.get("A_cause"):
                report += f"  [{ts}] 因果: {causal['A_cause']}\n"
            if causal.get("self_reflection"):
                report += f"  [{ts}] 自我反思: {causal['self_reflection'][:100]}...\n"
            if causal.get("recent_tensions"):
                for t in causal["recent_tensions"][-2:]:
                    report += f"    └ 张力: 资源'{t['resource_id']}' @ {t['weight']:.2f}\n"
